  ...
  return (
    <div className="bg-white/60 backdrop-blur-xl rounded-3xl p-8 shadow-xl border border-white/20">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-800">Requisições Salvas</h2>
      ...
      {loading && (
        <div className="flex justify-center items-center py-10">
          ...
          <p className="ml-4 text-gray-600">Buscando requisições...</p>
        </div>
      )}
      ...
      {!loading && !error && (
        <div className="space-y-4">
          {requests.length > 0 ? (
            requests.map(req => (
              <div key={req.id} className="bg-white rounded-2xl p-4 shadow-md border border-gray-200">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="font-bold text-lg text-blue-700">WEB ID: {req.webId}</p>
                    <p className="text-sm text-gray-600">Projeto: {req.contaProjeto}</p>
                ...
                <div className="mt-3 pt-3 border-t">
                  <p className="text-xs font-medium text-gray-500 mb-1">Passageiros nesta requisição:</p>
            ...
            <p className="text-center text-gray-500 py-10">Nenhuma requisição encontrada.</p>
          )}
        </div>
      )}
    </div>
