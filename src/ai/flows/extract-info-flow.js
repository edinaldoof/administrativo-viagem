// src/ai/flows/extract-info-flow.js
/**
...
export async function extractInfoFromPdf(input) {
  ...
  async (input) => {
    ...
    const processedOutput = {
      ...
      passengers: output.passengers.map(p => ({
        ...p,
        birthDate: parseDate(p.birthDate) || new Date(),
        itinerary: (p.itinerary || []).map(i => ({
          ...i,
          departureDate: parseDate(i.departureDate) || new Date(),
          returnDate: parseDate(i.returnDate) || undefined,
        }))
      }))
    };
